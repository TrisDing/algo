...
If we still shifting up, it might end up leaving a hole in the heap and thus breaks the complete binary tree rule. For example, delete (`33`):
```
...
Assuming the heap is min-heap
| Operation             | Time Complexity |
| --------------------- | :-------------: |
| find-min (top)        | O(1)            |
| delete-min (siftdown) | O(logn)         |
| Insert (siftup)       | O(logn)         |
| K largest             | O(nlogK)        |
| K smallest            | O(nlogK)        |
...
Python heapq is by default **min-heap**, create **max-heap** by push `-num` and negate again `-num` when pop:
```
h = []
for num in nums:
...
- Priority Queue
- The Top-K Problem
- Calculate Median (Use 2 heaps: a min-heap and a max-heap)
