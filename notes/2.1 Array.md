...
```py
"""
Modifies a list so that each index has a product of all the numbers present
in the list except the number itself stored at that index.
"""
nums = [1,2,3,4]
n = len(nums)
product = []
